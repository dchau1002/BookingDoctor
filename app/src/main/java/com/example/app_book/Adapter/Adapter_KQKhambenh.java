...
public class Adapter_KQKhambenh extends RecyclerView.Adapter<Adapter_KQKhambenh.Myholder> {
    Context context;
    List<Model_KQKham> userList;
    int countcmt = 0;
    private DatabaseReference benhanh,benhanh2;
    private DatabaseReference user;
    public Adapter_KQKhambenh(Context context, List<Model_KQKham> userList) {
    ...
    @NonNull
    @Override
    public Adapter_KQKhambenh.Myholder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.item_ketquakham, parent,false);

        return new Adapter_KQKhambenh.Myholder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull Adapter_KQKhambenh.Myholder holder, int i) {
